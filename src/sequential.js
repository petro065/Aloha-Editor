/**
 * sequential.js is part of Aloha Editor project http://aloha-editor.org
 *
...
define([
	...
	function remove(sequence, start, end) {
		...
		return {
			...
			ranges  : sequence.ranges.reduce(function (list, item) {
				var a = item[0];
				var b = item[1];
	...
	if (TESTS) {
		[{
			...
			var sequence = parseDom(Html.parse(test.markup, document)[0]);
			...
			sequence.ranges.forEach(function (range) {
	...
	function extractCollapsed(sequence) {
		var content = sequence.content;
		var collapsed = [];
		var offset = 0;
		var guard = 99;
	...
	if (TESTS) {
		[{
			...
			var sequence = parseDom(Html.parse(test.markup, document)[0]);
			var sequence2 = extractCollapsed(sequence);
			console.log('|' + sequence2.content + '|');
			console.log(sequence2);
		});
	}

	function create(start, end) {
		var line = Dom.upWhile(Boundaries.container(start), function (node) {
			return !Html.hasLinebreakingStyle(node)
			    && !Dom.isEditingHost(node);
		});
		var offset = Boundaries.offset(start);
		var sequence = parseDom(line);
		sequence.ranges.push([offset, offset, 'boundary']);
		return extractCollapsed(sequence);
	}
